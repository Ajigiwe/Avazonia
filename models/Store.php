...
class Store extends Model {
    public function findBySlug(string $slug): array|false {
        $stmt=$this->db->prepare("SELECT st.*, s.seller_type, s.verification_level, s.is_verified, s.business_name, s.country_code as seller_country FROM stores st JOIN sellers s ON st.seller_id=s.id WHERE st.slug=? AND st.is_active=1 LIMIT 1");
        $stmt->execute([$slug]);
        return $stmt->fetch();
    }
    ...
    public function getFeatured(int $limit=6): array {
        $stmt=$this->db->prepare("SELECT st.*, s.seller_type, s.verification_level, s.is_verified, s.business_name FROM stores st JOIN sellers s ON st.seller_id=s.id WHERE st.is_active=1 AND st.is_featured=1 ORDER BY st.created_at DESC LIMIT ".(int)$limit);
        $stmt->execute();
        return $stmt->fetchAll();
    }
    public function getByType(string $sellerType, int $limit=6): array {
        $stmt=$this->db->prepare("SELECT st.*, s.seller_type, s.verification_level, s.is_verified FROM stores st JOIN sellers s ON st.seller_id=s.id WHERE s.seller_type=? AND st.is_active=1 ORDER BY s.is_verified DESC, st.created_at DESC LIMIT ".(int)$limit);
        $stmt->execute([$sellerType]);
        return $stmt->fetchAll();
    }
    public function getAll(int $limit=20,int $offset=0): array {
        $stmt=$this->db->prepare("SELECT st.*, s.seller_type, s.verification_level, s.is_verified FROM stores st JOIN sellers s ON st.seller_id=s.id WHERE st.is_active=1 ORDER BY st.is_featured DESC, st.created_at DESC LIMIT ".(int)$limit." OFFSET ".(int)$offset);
        $stmt->execute();
        return $stmt->fetchAll();
    }
